...
def train_model(df, company, n_predict):
...
def evaluate_model(df, company, n_predict):
    ...
    prediction = prediction.append({'company_id': str(company), 'accuracy': str(np.round(score, 2)), 'rmse': round(np.mean(rmse_list), 2), 'mae': round(np.mean(mae_list), 2)}, ignore_index=True)
    prediction.to_csv("predictions.csv", index=False)
    
    print(np.round(score, 2), round(np.mean(rmse_list), 2), round(np.mean(mae_list), 2))
    return np.round(score, 2), round(np.mean(rmse_list), 2), round(np.mean(mae_list), 2), test_X, scaler
...
if __name__ == "__main__":
    ...
    for company in company_list:
        for n_predict in predict_list:
            evaluate_model(df, company, n_predict)

# =============================================================================
#     for company in company_list:
#         for n_predict in predict_list:
#             train_model(df, company, n_predict)
# =============================================================================
